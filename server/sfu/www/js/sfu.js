...
function addRemoteTrack(event) {
  ...
  if (video)
      //Ignore
    return console.log(
        'addRemoteTrack() video already present for ' + stream.id);
  ...
  track.onended = (event) => {
    ...
    if (!video)
        //Ignore
      return console.log(
          'removeRemoteTrack() video not present for ' + stream.id);
...
async function connect(url, roomId, name, secret) {
  ...
  pc.ontrack = (event) => {
    ...
    if (isCryptoEnabled) {
      ...
      //Decrytp
      receiverStreams.readableStream.pipeThrough(transform).
      ...
      if (!nopublish) {
        ...
        for (const track of stream.getTracks()) {
          ...
          if (isCryptoEnabled) {
            ...
            //Encrypt
            senderStreams.readableStream.pipeThrough(senderTransformStream).
...
navigator.mediaDevices.getUserMedia({
  ...
  if (!supportsInsertableStreams)
    dialog.querySelector(
        '#key').parentElement.innerHTML = '<red>Your browser does not support insertable streams<red>';
  if (roomId) {
    dialog.querySelector('#roomId').
        parentElement.
        MaterialTextfield.
        change(roomId);
    supportsInsertableStreams &&
    dialog.querySelector('#key').parentElement.MaterialTextfield.change(key);
  ...
  dialog.querySelector('#random').addEventListener('click', function() {
    dialog.querySelector('#roomId').
        parentElement.
        MaterialTextfield.
        change(Math.random().toString(36).substring(7));
    dialog.querySelector('#name').
        parentElement.
        MaterialTextfield.
        change(Math.random().toString(36).substring(7));
    dialog.querySelector('#key').
        parentElement.
        MaterialTextfield.
        change(Math.random().toString(36).substring(7));
  });
